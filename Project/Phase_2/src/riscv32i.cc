...
// RV32M encodings (R-type: opcode=0x33)
// funct7 = 0x01 for M-extension ops
...
void cpu(arch_t mem[MEM_SIZE], volatile strb_t* pstrb) {
  ...
  PROGRAM_LOOP: while (true) {
    ...
    // Fetch
    //--------------------------------------------------------------------------------------------------------
    // (Optional safety) PC bounds check
    {
      arch_t pc_idx = pc >> 2;
      if (pc_idx >= MEM_SIZE) {
    ...
    switch (opcode) {
      ...
      case OPCODE_IA:
        switch (funcx) {
          ...
          case FUNCX_SLT:  res = (((ap_int<ARCH>) src1) < ((ap_int<ARCH>) src2)) ? 1 : 0; break;
          case FUNCX_SLTU: res = (((ap_uint<ARCH>) src1) < ((ap_uint<ARCH>) src2)) ? 1 : 0; break;

          default:
            // ---- RV32M: MUL ----
            // MUL is R-type with funct7=0x01 and funct3=0x0
            if ((opcode == OPCODE_R) && (func7 == FUNCT7_M) && (func3 == FUNC3_MUL)) {
              // MUL returns low 32 bits of product
              res = (ap_int<ARCH>)src1 * (ap_int<ARCH>)reg_file[rs2];
            } else {
      ...
      case OPCODE_IM:
        ...
        switch (func3) {
          case FUNC3_XB:  res = (ap_int<ARCH>) ((ap_int<8>)  (val(7,0)));  break;
          case FUNC3_XH:  res = (ap_int<ARCH>) ((ap_int<16>) (val(15,0))); break;
          case FUNC3_XBU: res = val(7,0);  break;
          case FUNC3_XHU: res = val(15,0); break;
      ...
      case OPCODE_S:
        switch (func3) {
          case FUNC3_XB: wstrb = 0x1; break;
          case FUNC3_XH: wstrb = 0x3; break;
          case FUNC3_XW: wstrb = 0xf; break;
        }
        res = src2;
        break;
