...
`ifndef SOFTMAX_ACCEL_SUBSYSTEM_MONITOR_SV
`define SOFTMAX_ACCEL_SUBSYSTEM_MONITOR_SV
...
class softmax_accel_subsystem_monitor extends uvm_component;

    softmax_accel_reference_model refm;
    softmax_accel_scoreboard scbd;

    `uvm_component_utils_begin(softmax_accel_subsystem_monitor)
    `uvm_component_utils_end

    uvm_analysis_imp_axi_wtr_gmem#(axi_pkg::axi_transfer, softmax_accel_subsystem_monitor) gmem_wtr_imp;
    uvm_analysis_imp_axi_rtr_gmem#(axi_pkg::axi_transfer, softmax_accel_subsystem_monitor) gmem_rtr_imp;
    uvm_analysis_imp_axi_wtr_control#(axi_pkg::axi_transfer, softmax_accel_subsystem_monitor) control_wtr_imp;
    uvm_analysis_imp_axi_rtr_control#(axi_pkg::axi_transfer, softmax_accel_subsystem_monitor) control_rtr_imp;

    virtual function void build_phase(uvm_phase phase);
        super.build_phase(phase);
        if (!uvm_config_db#(softmax_accel_reference_model)::get(this, "", "refm", refm))
            `uvm_fatal(this.get_full_name(), "No refm from high level")
        `uvm_info(this.get_full_name(), "get reference model by uvm_config_db", UVM_MEDIUM)
        scbd = softmax_accel_scoreboard::type_id::create("scbd", this);
    endfunction
