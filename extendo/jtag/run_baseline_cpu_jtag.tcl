...
proc usage {} {
    puts "Usage: xsdb run_baseline_cpu_jtag.tcl ?options?"
    puts ""
    puts "Options:"
    puts "  --bit <path>         Path to the baseline CPU .bit file."
    puts "  --program <path>     Path to the RISC-V program hex text file."
    puts "  --inputs <vals...>   Decimal input logits."
...
proc default_inputs {count} {
    ...
    for {set i 0} {$i < $count} {incr i} {
        lappend values [expr {0.3 * ($i + 1)}]
    }
    return $values
}

proc parse_args {argv repo_root} {
    set opts(bit) [file join $repo_root vivado build pynq_z1 overlay baseline_cpu_pynq_z1.bit]
    set opts(program) [file join $repo_root softmax_naive_shift_scalable.txt]
...
proc wait_for_done {timeout_ms} {
    ...
    error "Baseline CPU timed out after ${timeout_ms}ms"
}
...
proc main {argv} {
    ...
    set ps7_init_path [file join $::repo_root vivado build pynq_z1 \
        baseline_cpu_pynq_z1.gen sources_1 bd baseline_cpu_pynq_z1 ip \
        baseline_cpu_pynq_z1_processing_system7_0_0 ps7_init.tcl]
    ...
    set ps_name [select_target_by_patterns $::PS_TARGET_PATTERNS]
    puts stderr "Using PS target: $ps_name"
    rst -system
    after 1000
    ...
    puts stderr "Starting baseline CPU..."
    pulse_start
