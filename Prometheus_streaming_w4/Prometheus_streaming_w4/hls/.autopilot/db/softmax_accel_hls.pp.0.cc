...
#pragma HLS INLINE
 if (sum_q16 == 0) return 0x7FFFFFFF;
  ...
  int msb = 31;
  VITIS_LOOP_98_1: for (int b = 31; b >= 0; --b) {
#pragma HLS UNROLL
 if (sum_q16[b]) {
      msb = b;
      break;
...
#pragma HLS INLINE
 int msb = 31;
  VITIS_LOOP_433_1: for (int b = 31; b >= 0; --b) {
#pragma HLS UNROLL
 if (sum_q16[b]) {
      msb = b;
      break;
...
extern "C" {
__attribute__((sdx_kernel("softmax_accel", 0))) void softmax_accel(volatile axi_vec_t *mem,
                   uint32_t input_base,
                   uint32_t prob_base,
                   uint32_t dbg_base,
                   uint32_t n) {
