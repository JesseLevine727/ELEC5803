...
void softmax_accel_core(axi_vec_t mem[((1 << 16) / 4)],
                        ...
                        uint32_t dbg_base,
                        uint32_t n);
void prometheus_soc(axi_vec_t mem[((1 << 16) / 4)], volatile strb_t* pstrb);
# 4 "softmax_accel_hls.cc" 2
...
#pragma HLS INLINE
 if (sum_q16 == 0) return 0x7FFFFFFF;
  ...
  int msb = 31;
  VITIS_LOOP_84_1: for (int b = 31; b >= 0; --b) {
#pragma HLS UNROLL
 if (sum_q16[b]) {
      msb = b;
      break;
...
static void load_logits_and_find_max(
    axi_vec_t mem[((1 << 16) / 4)],
    uint32_t in_l,
    uint32_t n,
    ap_int<32> logits_buf[256],
    ap_int<32> *max_x_out) {
...
#pragma HLS INLINE
 int msb = 31;
  VITIS_LOOP_410_1: for (int b = 31; b >= 0; --b) {
#pragma HLS UNROLL
 if (sum_q16[b]) {
      msb = b;
      break;
...
#pragma HLS BIND_STORAGE variable=exp_buf type=ram_2p impl=bram
 ...
 ap_int<32> max_x = 0;
  ...
  axi_vec_t dbg_line0 = mem[(db_w + 0) >> 2];
  pack_word(dbg_line0, (int)((db_w + 0) & 0x3), (ap_uint<32>)max_x);
  pack_word(dbg_line0, (int)((db_w + 1) & 0x3), sum_q16);
  pack_word(dbg_line0, (int)((db_w + 2) & 0x3), (ap_uint<32>)derive_shift_dbg(sum_q16));
  mem[(db_w + 0) >> 2] = dbg_line0;
}
